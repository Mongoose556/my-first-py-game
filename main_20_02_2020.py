...
class Unit:
    ...
    def attack(self, roll, target_unit):
        ...
        if attack_result == 1 or 2: #"Hit!" or "Hit and retreat!":
            target_unit.hit()
            print(target_unit.name, f" hit count: {target_unit.hits_remaining} ")

        if target_unit.is_alive == False: #e.status = -1
            print("%s Destroyed!" %target_unit.name)
        #if attack_result == "Destroyed!":
        #    target_unit.is_alive() = -1
...
#this loop pits one unit against another until destroyed or out of turns. 
#need to change it so unit take turns to attack each other
    ...
    while num_turns > 0:
        ...
        while enemy.is_alive() == True:
            #print(player.name, "rating: ", player.attack_rating)
            #print(enemy.name, "rating: ", enemy.attack_rating)
